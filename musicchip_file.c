/* vi:set ts=4 sts=4 sw=4 noexpandtab: */

/*()_)()_)()_)()_)()_)()_)()_)()_)()_
 / the MUSICCHIP file format by LFT /
 ()_)()_)()_)()_)()_)()_)()_)()_)()_*/

#include <stdio.h>
#include <string.h>

#include "pineapple.h"
#include "musicchip_file.h"

void savefile(char *fname, PT_TUNE *pt){
	FILE *f;
	int i, j;

	f = fopen(fname, "w");
	if(!f){
		fprintf(stderr, "save error!\n");
		return;
	}

	fprintf(f, "musicchip tune\n");
	fprintf(f, "version 1\n");
	fprintf(f, "\n");
	fprintf(f, "%s\n", filename);
	fprintf(f, "\n");
	fprintf(f, "#%s\n", pt->comment);
	fprintf(f, "\n");
	fprintf(f, "tempo: %d\n", pt->callbacktime);
	for(i = 0; i < pt->songlen; i++){
		fprintf(f, "songline %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",
			i,
			pt->song[i].track[0],
			pt->song[i].transp[0],
			pt->song[i].track[1],
			pt->song[i].transp[1],
			pt->song[i].track[2],
			pt->song[i].transp[2],
			pt->song[i].track[3],
			pt->song[i].transp[3]);
	}
	fprintf(f, "\n");
	for(i = 1; i < 256; i++){
		for(j = 0; j < tracklen; j++){
			struct trackline *tl = &pt->track[i].line[j];

			if(tl->note || tl->instr || tl->cmd[0] || tl->cmd[1]){
				fprintf(f, "trackline %02x %02x %02x %02x %02x %02x %02x %02x\n",
					i,
					j,
					tl->note,
					tl->instr,
					tl->cmd[0],
					tl->param[0],
					tl->cmd[1],
					tl->param[1]);
			}
		}
	}
	fprintf(f, "\n");
	for(i = 1; i < 256; i++){
		if(pt->instrument[i].length > 1){
			for(j = 0; j < pt->instrument[i].length; j++){
				fprintf(f, "instrumentline %02x %02x %02x %02x\n",
					i,
					j,
					pt->instrument[i].line[j].cmd,
					pt->instrument[i].line[j].param);
			}
		}
	}

	fclose(f);
}

PT_TUNE *loadfile(char *fname){
	FILE *f;
	char buf[1024];
	int cmd[3];
	int i1, i2, trk[4], transp[4], param[3], note, instr;
	int i;
	char c;
	PT_TUNE *pt;

	snprintf(filename, sizeof(filename), "%s", fname);

	f = fopen(fname, "r");
	if(!f){
		return NULL;
	}

	pt->songlen = 1;
	while(!feof(f) && fgets(buf, sizeof(buf), f)){
		if(1 == sscanf(buf, "#%1024c", &pt->comment)){
		}
		else if(1 == sscanf(buf, "tempo: %hhd", &pt->callbacktime)){
			pt->callbacktime = (u8)pt->callbacktime;
		}else if(9 == sscanf(buf, "songline %x %x %x %x %x %x %x %x %x",
			&i1,
			&trk[0],
			&transp[0],
			&trk[1],
			&transp[1],
			&trk[2],
			&transp[2],
			&trk[3],
			&transp[3])){

			for(i = 0; i < 4; i++){
				pt->song[i1].track[i] = trk[i];
				pt->song[i1].transp[i] = transp[i];
			}
			if(pt->songlen <= i1) pt->songlen = i1 + 1;
		}else if(8 == sscanf(buf, "trackline %x %x %x %x %x %x %x %x",
			&i1,
			&i2,
			&note,
			&instr,
			&cmd[0],
			&param[0],
			&cmd[1],
			&param[1])){

			pt->track[i1].line[i2].note = note;
			pt->track[i1].line[i2].instr = instr;
			for(i = 0; i < 2; i++){
				pt->track[i1].line[i2].cmd[i] = cmd[i];
				pt->track[i1].line[i2].param[i] = param[i];
			}
		}else if(4 == sscanf(buf, "instrumentline %x %x %x %x",
			&i1,
			&i2,
			&cmd[0],
			&param[0])){

			pt->instrument[i1].line[i2].cmd = cmd[0];
			pt->instrument[i1].line[i2].param = param[0];
			if(pt->instrument[i1].length <= i2) pt->instrument[i1].length = i2 + 1;
		}
	}

	fclose(f);
	return pt;
}
