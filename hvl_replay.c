...
void hvl_process_stepfx_1( struct hvl_tune *ht, struct hvl_voice *voice, int32 FX, int32 FXParam )
{
  switch( FX )
  {
    ...
    case 0xb: // Position jump
      ...
      if( ht->ht_PosJump <= tune->songpos )
        ht->ht_SongEndReached = 1;
      break;

    case 0xd: // Pattern break
      ht->ht_PosJump      = tune->songpos+1;
...
void hvl_process_step( struct hvl_tune *ht, struct hvl_voice *voice )
{
  ...
  Step = &ht->ht_Tracks[ht->ht_Positions[tune->songpos].pos_Track[voice->vc_VoiceNum]][ht->ht_NoteNr];
...
void hvl_play_irq( struct hvl_tune *ht )
{
  ...
  if( ht->ht_StepWaitFrames <= 0 )
  {
    ...
    {
      int32 nextpos = (tune->songpos+1==ht->ht_PositionNr)?0:(tune->songpos+1);

      for( i=0; i<ht->ht_Channels; i++ )
      {
        ht->ht_Voices[i].vc_Track         = ht->ht_Positions[tune->songpos].pos_Track[i];
        ht->ht_Voices[i].vc_Transpose     = ht->ht_Positions[tune->songpos].pos_Transpose[i];
  ...
  if( ht->ht_Tempo > 0 && --ht->ht_StepWaitFrames <= 0 )
  {
    ...
    {
      ht->ht_PatternBreak = 0;
      tune->songpos       = ht->ht_PosJump;
      ht->ht_NoteNr       = ht->ht_PosJumpNote;
      if( tune->songpos == ht->ht_PositionNr )
      {
        ht->ht_SongEndReached = 1;
        tune->songpos         = ht->ht_Restart;
      }
      ht->ht_PosJumpNote  = 0;
      ht->ht_PosJump      = 0;
...
int hvl_save_hvl(struct hvl_tune *ht, sturct pineapple_tune *pt){
}
*/
